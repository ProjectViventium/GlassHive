...
from workers_projects_runtime.run_actions import mint_run_action_capability, unverified_run_action_claims
    ...
    worker = store.create_worker(
        ...
        bootstrap_bundle={
            "callbacks": {
                ...
                "logical_turn_id": "turn-synthetic",
...
def test_retryable_failed_callback_carries_signed_short_lived_retry_capability(tmp_path, monkeypatch):
    ...
    with TestClient(app):
        ...
        assert payload["logical_turn_id"] == "turn-synthetic"
        assert payload["logical_turn_revision"] == "2"
...
def test_action_capability_rejects_signature_tampering_and_changed_db_owner(tmp_path, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        canonical_index = alphabet.index(token[-1])
        assert canonical_index % 4 == 0
        replacement = alphabet[canonical_index + 1]

        tampered = client.post(
            "/v1/run-actions",
            json=body,
            headers={"X-Viventium-Action-Capability": token[:-1] + replacement},
        )
...
def test_cancel_replay_waits_for_expired_exact_claim_recovery_without_false_acceptance(
    ...
    with TestClient(app) as client:
        ...
        with app.state.store._connect() as conn:  # synthetic reaper clock advance
            conn.execute(
