...
async function initialize() {
  ...
  detail('#confirm-scopes', isAccountChange
  ...
  optionalDetail('#confirm-version-row', '#confirm-version', librarySnapshot.version || '');
  optionalDetail(
    '#confirm-provenance-row',
    '#confirm-provenance',
    [provenance.publisher, provenance.source, provenance.revision].filter(Boolean).join(' · '),
  );
  optionalDetail('#confirm-hash-row', '#confirm-hash', librarySnapshot.content_hash || '');
  optionalDetail('#confirm-dependencies-row', '#confirm-dependencies', dependencies.join('; '));
  document.querySelector('#confirm-details').hidden = false;
  document.querySelector('#confirm-intro').textContent = isAccountChange
